...
<div class="p-4 sm:p-6 bg-gray-50 min-h-screen">
    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-x-6 md:gap-y-6">
        ...
        {{-- Chart dan IPK --}}
        <div class="col-span-12 md:col-span-8 bg-yellow-100 p-4 h-auto md:h-95 rounded-xl shadow transform-gpu transition-transform duration-200 ease-out hover:scale-[1.01] hover:shadow-lg motion-reduce:transform-none motion-reduce:transition-none">
        ...
        <div class="relative overflow-hidden col-span-12 md:col-span-4 bg-gradient-to-br from-yellow-50 to-yellow-100 p-6 md:p-15 md:pt-20 h-auto md:h-95 rounded-xl shadow text-center transform-gpu transition-transform duration-200 ease-out hover:scale-[1.01] hover:shadow-lg ring-1 ring-yellow-200/70 motion-reduce:transform-none motion-reduce:transition-none">
            <div class="pointer-events-none absolute -top-10 -right-10 w-40 h-40 bg-yellow-200/40 rounded-full blur-2xl"></div>
            <div class="pointer-events-none absolute -bottom-12 -left-8 w-48 h-48 bg-yellow-300/30 rounded-full blur-3xl"></div>
            ...
            <div class="mx-auto w-28 h-28 relative mb-3">
                <div class="absolute inset-0 rounded-full" style="background: conic-gradient(#F59E0B {{ $__pct }}%, #FDE68A {{ $__pct }}% 100%)"></div>
                <div class="absolute inset-2 rounded-full bg-white/70 backdrop-blur ring-1 ring-yellow-200/60"></div>
                <div class="relative z-10 flex items-center justify-center w-full h-full">
                    <div>
                        <p class="text-3xl font-extrabold text-yellow-600 leading-none">{{ number_format($__ipkCalc, 1) }}</p>
            ...
            <div class="flex items-center justify-center gap-2 mb-2">
                ...
                <span class="px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-yellow-200/70 text-yellow-800 border border-yellow-300/80">
                    {{ $badge }}
                </span>
            </div>
        ...
        {{-- Jadwal Hari Ini --}}
        <div class="col-span-12 bg-white p-4 sm:p-6 rounded-xl shadow transform-gpu transition-transform duration-200 ease-out hover:scale-[1.01] hover:shadow-lg motion-reduce:transform-none motion-reduce:transition-none">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            new Chart(canvas, {
                ...
                options: {
                    ...
                    layout: {
                        padding: { top: 28, right: 8, bottom: 8, left: 8 }
                    },
                    plugins: {
                        legend: { display: false },
                        tooltip: {
                            callbacks: {
                                label: (ctx) => `${ctx.parsed.y?.toFixed ? ctx.parsed.y.toFixed(1) : ctx.parsed.y}`
                            }
                        },
                        datalabels: {
