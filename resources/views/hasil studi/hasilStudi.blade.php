<x-app-layout>
    ...
    <div class="space-y-6 bg-gradient-to-br from-white-50 via-white to-white-100 p-4 sm:p-6 rounded-3xl shadow-lg border border-yellow-100">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            @php
                $stats = [
                    ['label' => 'Total SKS Semester ini', 'value' => $totalSKS, 'sub' => 'SKS yang diambil', 'icon' => 'fa-layer-group'],
        ...
        <div class="bg-white rounded-2xl border border-yellow-100 shadow-sm overflow-hidden transform-gpu transition-transform duration-200 hover:scale-[1.01] hover:shadow-lg">
            <div class="px-5 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-yellow-50 border-b border-yellow-100">
                ...
                <div class="flex gap-2">
                    <span class="px-3 py-1 rounded-full bg-white text-yellow-700 border border-yellow-200 text-xs font-semibold">
                        {{ $mahasiswaDetails->program_studi ?? '-' }}
                    </span>
                    <span class="px-3 py-1 rounded-full bg-white text-yellow-700 border border-yellow-200 text-xs font-semibold">
                        ...
                        @forelse ($krsData as $i => $krs)
                            <tr class="hover:bg-yellow-50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <span class="inline-flex mt-1 px-2 py-0.5 bg-yellow-100 text-yellow-800 text-xs font-semibold rounded-full">
                                        {{ $krs->kelas->mataKuliah->sks ?? 0 }} SKS
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-gray-700">
                                    {{ $krs->kelas->dosenPengampu->nama_lengkap ?? '-' }}
                                </td>
